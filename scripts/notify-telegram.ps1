...
.NOTES
  ...
  - alerts.json: { keywords:[...], logic:"OR"|"AND", recentHours:72, maxPerRun:30 }
#>
[CmdletBinding()]
param(
...
if(-not $firstRun){
  try{ foreach($l in @((Get-Content $alertedPath -Raw -Encoding UTF8 | ConvertFrom-Json).sent)){ if($l){ [void]$sent.Add($l) } } }catch{}
}

function Save-Alerted(){
  # 최근 5000건만 보존
  $arr = @($sent) | Select-Object -Last 5000
  ([ordered]@{ updated=(Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ'); sent=$arr } | ConvertTo-Json -Depth 5) |
    Set-Content -Path $alertedPath -Encoding UTF8
}

# 첫 실행: 현재 매칭분을 보낸 것으로 시드만 하고 종료(과거분 폭주 방지)
if($firstRun){
  foreach($m in $matched){ $key = if($m.link){$m.link}else{$m.title}; [void]$sent.Add($key) }
  Save-Alerted
